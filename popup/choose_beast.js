const texto = document.querySelector("#ducocantidad");
const boton = document.querySelector('#miBoton');
const textbox = document.querySelector('#nombre');

boton.addEventListener("click", function(e) {
  texto.innerHTML = "pulsado"
  nombre = textbox.value;
  fetch('https://server.duinocoin.com/balances.json')
      .then(result => result.json())
      .then((output) => {
        console.log('Output: ', output);
        texto.innerHTML = `${nombre}: ${output[nombre]}`;
      })
      .catch(err => console.error(err));
})

texto.innerHTML = "Duco: ¿?";
